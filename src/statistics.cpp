...
void init_alias_table(AliasTable *alias_table, u32 n, const f32 *weights, u64 seed) {
  alias_table->n = n;
  alias_table->rng = create_rng(seed);

  // set internal pointers correctly
  if (n > STATIC_ALIAS_THRESHOLD) {
    alias_table->dynamic_buffer = (u8 *)malloc(n * (sizeof(f32) + sizeof(u32)));
    assert(alias_table->dynamic_buffer);
    alias_table->probability_table = (f32 *)alias_table->dynamic_buffer;
    alias_table->alias_table = (u32 *)(alias_table->dynamic_buffer + n * sizeof(f32));
  } else {
    alias_table->dynamic_buffer = NULL;
    alias_table->probability_table = alias_table->static_probability_table;
    alias_table->alias_table = alias_table->static_alias_table;
  }
  ...
  for (u32 i = 0; i < n; i++) {
    alias_table->probability_table[i] = scale * weights[i];
    if (alias_table->probability_table[i] > 1.0f) {
      large[large_top++] = i;
    } else {
      small[small_top++] = i;
    }
  ...
  while (small_top > 0 && large_top > 0) {
    ...
    alias_table->alias_table[i_small] = i_large;

    // the excess probability of getting l moved into s
    f32 delta_p = 1.0f - alias_table->probability_table[i_small];
    alias_table->probability_table[i_large] = alias_table->probability_table[i_large] - delta_p;

    if (alias_table->probability_table[i_large] < 1.0f) {
  ...
  while (large_top > 0) {
    ...
    alias_table->probability_table[idx] = 1.0f;
    alias_table->alias_table[idx] = idx;
  }
  while (small_top > 0) {
...
void destroy_alias_table(AliasTable *alias_table) {
  if (alias_table->dynamic_buffer) {
    free(alias_table->dynamic_buffer);
    alias_table->dynamic_buffer = NULL;
  }
}
...
u32 draw_alias_table(AliasTable *alias_table) {
  ...
  f32 x = random_f32_xoroshiro128_plus(&alias_table->rng);

  // 2. let i = floor(n * x) and y = nx - i
  //    i uniform in [0, n - 1], y uniform in [0, 1)
  f32 nx = alias_table->n * x;
  u32 i = (u32)nx;
  f32 y = nx - i;

  // 3. if y < U_i, return i
  // else, return K_i
  if (y < alias_table->probability_table[i]) {
    return i;
  }
  return alias_table->alias_table[i];
}
...
void log_alias_table(const AliasTable *alias_table) {
  ...
  for (u32 i = 0; i < alias_table->n; i++) {
    printf("%d %f %d\n", i, alias_table->probability_table[i], alias_table->alias_table[i]);
  }
}
