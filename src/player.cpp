...
// PlayerOpenGLRenderData new_player_opengl_render_data(unsigned program, unsigned texture) {
...
//   glUniformBlockBinding(program, data.matrices_buffer_index, CAMERA_MATRICES_INDEX);
//
...
void opengl_draw_player(const PlayerOpenGLRenderData *render_data, const glm::mat4 &model) {
  glUseProgram(render_data->program);
  glBindVertexArray(render_data->vao);
  glBindTexture(GL_TEXTURE_2D, render_data->texture);
  glUniformMatrix4fv(render_data->u_model_location, 1, GL_FALSE, glm::value_ptr(model));
  glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
}

void update_player_position(Player *player, float delta_t, const glm::vec4 &movement_direction) {
