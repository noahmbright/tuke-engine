...
void push_subdirectory(SubdirectoryList *subdirectory_list, const char *s) {
  if (subdirectory_list->num_subdirectories >= MAX_NUM_SUBDIRECTORIES) {
    fprintf(stderr,
            "push_subdirectory: subdirectory_list exceed maximum capacity\n");
    exit(1);
  }

  snprintf(
      subdirectory_list->subdirectories[subdirectory_list->num_subdirectories],
      SUBDIRECTORY_PATH_BUFFER_LENGTH, "%s", s);
...
void collect_shaders_to_compile(const SubdirectoryList *subdirectory_list,
                                ShaderToCompileList *shader_to_compile_list) {
  for (u32 subdirectory_index = 0;
       subdirectory_index < subdirectory_list->num_subdirectories;
       subdirectory_index++) {

    const char *subdirectory_path =
        subdirectory_list->subdirectories[subdirectory_index];
    ...
    while (*cursor != '\0' &&
           (cursor - subdirectory_path) < SUBDIRECTORY_PATH_BUFFER_LENGTH) {
      cursor++;
    }
    ...
    if (!subdirectory) {
      fprintf(stderr, "collect_shaders_to_compile: Failed to open %s: %s",
              subdirectory_path, strerror(errno));
      continue;
    }
    ...
    while ((directory_entry = readdir(subdirectory))) {
      ...
      ShaderToCompile *shader_to_compile =
      ...
      int characters_to_copy =
          snprintf(full_path, sizeof(full_path), "%s/%s", subdirectory_path,
                   directory_entry->d_name);
      if (characters_to_copy < 0) {
        fprintf(stderr,
                "collect_shaders_to_compile: encoding error in snprintf'\n");
        continue;
      }
      if (characters_to_copy >= FULL_PATH_BUFFER_LENGTH) {
        fprintf(stderr,
                "collect_shaders_to_compile: snprintf truncated path %s'\n",
                full_path);
        continue;
      }
      ...
      while ((cursor - full_path) < FULL_PATH_BUFFER_LENGTH - 1 &&
             *cursor != '\0') {
        if (*cursor == '/') {
      ...
      if (*cursor == '\0') {
        fprintf(
            stderr,
            "collect_shaders_to_compile: shader filename %s ended after name, "
            "stil needed stage and extension\n",
            full_path);
        continue;
      } else {
        cursor++;
      ...
      if (*cursor == '\0') {
        fprintf(
            stderr,
            "collect_shaders_to_compile: shader filename %s ended after stage, "
            "stil needed extension\n",
            full_path);
        continue;
      } else {
        cursor++;
      ...
      if (*cursor != '\0') {
        fprintf(stderr,
                "collect_shaders_to_compile: shader filename %s not finished "
                "after extension\n",
                full_path);
        continue;
      }
      ...
      if (strcmp(shader_stage_buffer, "vert") == 0) {
        ...
        fprintf(stderr,
                "collect_shaders_to_compile: got an invalid shader stage %s\n",
                shader_stage_buffer);
        continue;
      }

      const char *shader_source =
          read_file(full_path, &shader_to_compile->source_length);

      if (shader_source == NULL) {
        fprintf(stderr,
                "collect_shaders_to_compile: Failed to get source for %s\n",
                full_path);
        continue;
      }
